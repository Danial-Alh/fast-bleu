...
def self_bleu_test():
    ...
    def nltk_bleu(refs, hyps):
        ...
        bleu = SelfBleu(refs, weights, smoothing_function=SmoothingFunction(epsilon=1. / 10).method0)
        return bleu.get_score()
        # return [1. for hyp in hyps]

    def cpp_bleu(refs, hyps):
        from lib.self_bleu import SelfBleu
        bleu = SelfBleu(refs, weights, smoothing_function=0, other_instance=SelfBleu(refs, [0.5, 0.5]))
